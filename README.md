...
- Adjusts each slave’s `currentTime` or `playbackRate` to keep them aligned
...
### Key Components
- **FFmpeg** → Reads input (RTSP or MP4) and generates HLS
- **Node + Express** → Serves HLS segments on port 8000
- **React Dashboard** → Plays 5 streams with sync control
